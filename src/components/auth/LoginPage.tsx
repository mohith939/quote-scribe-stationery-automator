...
export function LoginPage({ onLogin, onGoogleLogin }: LoginPageProps) {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      toast({
        title: "Google Login",
        description: "Simulating Google OAuth login...",
      });
      
      // Simulate Google OAuth flow with proper delay
        ...
        <Card className="backdrop-blur-xl bg-white/80 border-slate-200/60 shadow-xl">
          ...
          <CardContent className="space-y-6">
            ...
            >
              {isGoogleLoading ? (
                <>
                  <div className="mr-3 h-5 w-5 animate-spin rounded-full border-2 border-slate-300 border-t-slate-600"></div>
